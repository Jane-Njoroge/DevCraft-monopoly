...
def create_player():
    ...
    db.session.commit()
    return jsonify({"message": "Player created!", "player": {"name": new_player.name, "balance": new_player.balance}}), 201

@app.route('/players', methods=['GET'])
def get_players():
    players = Player.query.all()
    return jsonify([{ "id": p.id, "name": p.name, "balance": p.balance, "position": p.position } for p in players])
...
def roll_dice(player_id):
    ...
    db.session.commit()
    return jsonify({"message": "Dice rolled", "dice": [dice1, dice2], "new_position": player.position})
...
def pay_rent():
    ...
    owner.balance += rent
    db.session.commit()
    return jsonify({"message": "Rent paid", "payer_balance": payer.balance, "owner_balance": owner.balance})
